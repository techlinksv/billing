...
export const IncomeReport: React.FC<{ title?: string }> = ({ title }) => {
  ...
  const handleDownload = (ev: React.MouseEvent) => {
    ...
    dispatch(
      downloadFile({
        endpoint: '/report/income-export',
        params: `?type=ingreso&from=${from}&to=${to}${searchString}`,
        name: `Ingresos.xlsx`,
      })
    )
  }
  ...
  return (
    <>
      ...
      <DataGrid
        key={`transactions-list-${idx}-ingreso`}
        url={`/transactions?type=ingreso&from=${from}&to=${to}`}
        columns={[
          {
            id: 'date',
            name: 'Fecha',
